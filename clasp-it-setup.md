...
\outl0\strokewidth0 \strokec2 # Clasp-it \'97 Setup Guide\
\
> **Ask me anything about Clasp-it.** Drop this file into your project and ask your AI editor: _"How do I use Clasp-it?"_ or _"Why isn't my MCP connection working?"_ \'97 they'll have everything they need to help you.\
\
---\
\
## What is Clasp-it?\
...
## Installation\
\
### Step 1 \'97 Load the Chrome extension\
\
...
2. Unzip it to a **permanent folder** \'97 do not delete this folder after loading, Chrome needs it to stay there\
3. Open `chrome://extensions` in Chrome\
4. Enable **Developer mode** using the toggle in the top-right corner\
5. Click **Load unpacked** and select the unzipped folder\
6. The Clasp-it icon will appear in your extensions bar \'97 pin it for easy access\
\
### Step 2 \'97 Add your API key to the extension\
\
1. Click the Clasp-it icon in the toolbar \'97 the side panel opens\
2. Enter your email and follow the magic link, **or** paste your API key directly if you have it\
3. Your API key is in your welcome email \'97 it starts with `cit_`\
\
### Step 3 \'97 Connect your AI editor via MCP\
\
Open the extension sidebar \uc0\u8594  Settings (gear icon) \u8594  **MCP server setup**. Select your editor for the exact setup command.\
\
**Claude Code** \'97 run once in your terminal:\
```bash\
claude mcp add --scope user --transport http clasp-it https://claspit.dev/mcp --header "Authorization: Bearer YOUR_API_KEY"\
```\
\
**Cursor** \'97 add to `~/.cursor/mcp.json`:\
```json\
\{\
  "mcpServers": \{\
    "clasp-it": \{\
...
```\
\
**Windsurf** \'97 add to `~/.codeium/windsurf/mcp_config.json`:\
```json\
\{\
  "mcpServers": \{\
    "clasp-it": \{\
...
4. Hover over elements \'97 they highlight as you move\
...
- _"The button I picked \'97 change its variant to primary"_\
\
---\
\
## Plans\
...
| DOM & Computed Styles | \uc0\u9989  | \u9989  |\
| Screenshot | \'97 | \uc0\u9989  |\
| Console logs | \'97 | \uc0\u9989  |\
| Network requests | \'97 | \uc0\u9989  |\
| React props | \'97 | \uc0\u9989  |\
...
- Try signing out (gear icon \uc0\u8594  Sign out) and re-entering the key\
\
### MCP not responding in your editor\
1. Test your key: `curl https://claspit.dev/auth/info -H "Authorization: Bearer YOUR_KEY"`\
2. If you get `\{"error":"Invalid API key"\}` \'97 re-authenticate in the extension\
3. If you get `\{"error":"Missing X-API-Key or Authorization header"\}` \'97 the MCP command wasn't set up correctly. Remove and re-add using the setup instructions in the extension settings.\
4. Restart your editor after making changes\
\
### Extension not capturing elements on a page\
- Refresh the page first \'97 the content script sometimes needs a fresh injection\
- Chrome built-in pages (`chrome://`, `chrome-extension://`) cannot be injected \'97 this is a browser restriction\
...
Pick context (HTML, CSS, your prompt) is stored in Redis with a 24-hour TTL, then deleted automatically. Screenshots are never stored server-side \'97 they are captured, sent in the POST request, and discarded.\
\
...
Yes \'97 Clasp-it works on any page Chrome can load, including `http://localhost:3000`.\
\
...
\
We read every message \'97 especially bug reports and feature requests from beta users.\
}